...
now = datetime.now() #définit quand review cette nouvelle carte (mtn ici)
...
#194 : (1.14, 1.01, 5.44, 14.67, 5.3024, 1.5662, 1.2503, 0.0028, 1.5489, 0.1763, 0.9953, 2.7473, 0.0179, 0.3105, 0.3976, 0.0, 2.0902) 
...
class Carte_Langue:
    ...
    def __init__(self,id="1",category="cat",front="front",subFront="subfront",back="back",meanings="meanings",subBack="suback",tag="tag") -> None: #subBack1 = meaning, je sais pas trop comment gérer pour l'instant
        self.card = Card()
        self.scheduling_cards = f.repeat(self.card, datetime.now())
        Carte_Langue.listeCartes.append(self)
        self.id = id
        self.category = category
        self.front = front
        self.subFront = subFront
        self.back = back
    ...
    def rateDebug(self,rating):
        Carte_Langue.rate(self,rating)
        print(self.timeBeforeReview())
        print(self.card.scheduled_days)

    def timeBeforeReview(self):
        delta = self.card.due - datetime.now()
        return delta.total_seconds()
    
    def resetDueTime(self): 
        self.card.due = datetime.now()
